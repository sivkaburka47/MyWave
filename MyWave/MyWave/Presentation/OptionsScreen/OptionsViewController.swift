...
extension OptionsViewController {
    ...
    private func configureFaceIdStack() {
        ...
        faceIdLabel.snp.makeConstraints {
            $0.left.equalTo(faceIdIcon.snp.right).offset(Constants.smallSpacing)
            $0.centerY.equalToSuperview()
        }
    ...
    private func bindViewModel() {
        viewModel.onRemindersUpdated = { [weak self] reminders in
            DispatchQueue.main.async {
                self?.updateRemindersContainer(with: reminders)
            }
        }

        viewModel.onReminderSwitchChanged = { [weak self] isOn in
            DispatchQueue.main.async {
                self?.reminderSwitch.isOn = isOn
            }
        }

        viewModel.onFaceIdSwitchChanged = { [weak self] isOn in
            DispatchQueue.main.async {
                self?.faceIdSwitch.isOn = isOn
            }
        }
    ...
    private func updateRemindersContainer(with reminders: [String]) {
        remindersContainer.arrangedSubviews.forEach { $0.removeFromSuperview() }
        for (index, time) in reminders.enumerated() {
            let reminderView = ReminderView(time: time)
            reminderView.buttonAction = { [weak self] in
                self?.viewModel.removeReminder(at: index)
            }
...
extension OptionsViewController {
    @objc private func addReminder() {
        ...
        let okAction = UIAlertAction(title: Metrics.Strings.addActionTitle, style: .default) { [weak self] _ in
            ...
            self?.viewModel.addReminder(time: timeString)
        }
    ...
    @objc private func reminderSwitchChanged(_ sender: UISwitch) {
        viewModel.toggleReminderSwitch(isOn: sender.isOn)
    }

    @objc private func faceIdSwitchChanged(_ sender: UISwitch) {
        viewModel.toggleFaceIdSwitch(isOn: sender.isOn)
    }
}
